...
let camLookX: number; // X position the camera will look at
let camLookY: number; // Y position the camera will look at
let camLookZ: number; // Z position the camera will look at
let currLookY: number; // Current Y position the camera is looking at
let currLookZ: number; // Current Z position the camera is looking at

// These variables store object instances that are referenced later.
let player: InstanceType.Player;
let arrow: InstanceType.ArrowPart1;
let arrow2: InstanceType.ArrowPart2;
let bowUI: InstanceType.BowUI;
let crosshairUI: InstanceType.CrosshairUI;
let scoreText: InstanceType.ScoreText;
...
// List of instances
let playerPos: InstanceType.PlayerPos[];
let shotCalcs: InstanceType.ShotCalculator[];
let tree: InstanceType.Tree[];
let bush: InstanceType.Bush[];

// Textures
let tex;

// Special objects
let mouseLocker: Element; // Canvas to request mouse lock

// Gameplay variables
let aOff: number; // Aim offset
let arrowLaunched: boolean; // Has the arrow been launched?
let currTarget: InstanceType.ShotCalculator; // Target the player is currently facing
let score: number; // Current player score
let breathingPattern: number; // How the camera moves when the player breathes
...
function setupBillboards(runtime: IRuntime) {
...
function aimCurve(sign: number, sens: number) {
...
function setCamera3DArrow(runtime: IRuntime) {
