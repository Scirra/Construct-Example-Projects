...
let backgroundTable: InstanceType.BackgroundTable;
let clay: InstanceType.Clay;
let clayShadow: InstanceType.ClayShadow;
let stoneBase: InstanceType.StoneBase;
let woodenBase: InstanceType.WoodenBase;
let canvas: InstanceType.DrawingCanvas;
let brush: InstanceType.Brush;
let toolCase: InstanceType.ToolCase;
let pickerBody: InstanceType.PickerBody;
let buttonReady: InstanceType.ButtonReady;
let buttonDelete: InstanceType.ButtonDelete;
let buttonBlur: InstanceType.ButtonBlur;
let buttonScreenshot: InstanceType.ButtonScreenshot;
let buttonReset: InstanceType.ButtonReset;
let screenshotGroup: InstanceType.ScreenshotGroup;
let fader: InstanceType.Fader;
let startGame: InstanceType.StartGame
let tutorialText: InstanceType.TutorialText;

// List of instances.
let tools: InstanceType.Tool[];
let colorPickers: InstanceType.ColorPicker[];
let buttonsBackground: InstanceType.ButtonBackground[];

// Gameplay variables.
let runtime: IRuntime; // Globally accessible runtime variable.
let sTool: InstanceType.Tool | null = null; // Selected tool.
let rotationSpeed: number = 1200; // Base's rotation speed.
let transitionPlaying: boolean = false; // Is a transition currently playing?
let backgroundSelected: number = 0; // Currently selected background.
...
function onToolDragStart(e: BehaviorInstanceEvent<InstanceType.Tool, IDragDropBehaviorInstance<InstanceType.Tool>>) {
...
function toggleBlur() {
	...
	background.effects[0].setParameter(0, Math.abs((background.effects[0].getParameter(0) as number) - 1))
	background.effects[1].setParameter(0, Math.abs((background.effects[1].getParameter(0) as number) - 1))
}
...
function pickColor() {
	...
	for (const cp of colorPickers) {
		if (brush.testOverlap(cp)) {
			...
			(brush.getParent()! as ISpriteInstance).animationFrame = parseInt(cp.colorRgb.join(''), 2)
		}
	}
}
...
function sculptClay(chiselNumber: number) {
...
function client2Layer(e: MouseEvent): Vec2Arr {
...
function mouseOverObj(c: IWorldInstance, e: MouseEvent): boolean {
