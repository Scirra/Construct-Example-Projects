...
export class RoomLayoutManager
{
	...
	multiplayer: IMultiplayerObjectType;
	signallingStatus: InstanceType.SignallingStatus;
	peerList: InstanceType.PeerList;
	receivedMessage: InstanceType.ReceivedMessage;

	messageTextInput: InstanceType.MessageText;
	...
	constructor(runtime: IRuntime)
	{
		...
		this.eventHandlers = new MultiEventHandler([
			[this.multiplayer,		"peerconnect",		(e: MultiplayerPeerEvent) => this.OnPeerConnect(e)],
			[this.multiplayer,		"peerdisconnect",	(e: MultiplayerPeerDisconnectEvent) => this.OnPeerDisconnect(e)],
			[this.multiplayer,		"message",			(e: MultiplayerMessageEvent) => this.OnMessage(e)],
	...
	SetSignallingStatus(str: string)
	{
		this.signallingStatus.text = "Signalling status: " + str;
	}
	
	// When peers connect or disconnect, update the list of peers.
	OnPeerConnect(e: MultiplayerPeerEvent)
	{
		this.UpdatePeerList();
	}
	
	OnPeerDisconnect(e: MultiplayerPeerDisconnectEvent)
	{
		this.UpdatePeerList();
	}
	...
	OnMessage(e: MultiplayerMessageEvent)
	{
		...
		else
		{
			...
			const dataView = new DataView(e.message as ArrayBuffer);
	...
	Send(content: MultiplayerMessageType)
	{
