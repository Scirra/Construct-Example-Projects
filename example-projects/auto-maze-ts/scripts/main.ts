...
let camBody: InstanceType.CameraBody; // Body that holds the main camera
let fader: InstanceType.Fader; // Fade sprite

// These are names to access IObjectInterfaces
let camera: I3DCameraObjectType; // Camera3D
let props: IObjectType<InstanceType.Prop>; // Prop
let walls: IObjectType<InstanceType.Wall>; // Wall

// Texture-related variables
let propTextures: [     // Prop textures
    IObjectType<InstanceType.TextureProp00>,
    IObjectType<InstanceType.TextureProp01>,
    IObjectType<InstanceType.TextureProp02>,
    IObjectType<InstanceType.TextureProp03>,
    IObjectType<InstanceType.TextureProp04>,
]; 
let worldTextures: [    // World textures (floor, wall, ceil)
        {
            "floor": IObjectType<InstanceType.TextureFloor00>,
            "wall": IObjectType<InstanceType.TextureWall00>,
            "ceil": IObjectType<InstanceType.TextureCeil00>
        },
        {
            "floor": IObjectType<InstanceType.TextureFloor01>,
            "wall": IObjectType<InstanceType.TextureWall01>,
            "ceil": IObjectType<InstanceType.TextureCeil01>
        },
        {
            "floor": IObjectType<InstanceType.TextureFloor02>,
            "wall": IObjectType<InstanceType.TextureWall02>,
            "ceil": IObjectType<InstanceType.TextureCeil02>
        }
];
let worldTexturesID = 0; // Current world textures set ID
...
type WorldEntry = {
    visited: boolean, // Has the tile been visited?
    left: Vec2Arr | number | null, // Does it have a neighbor to the left side?
    right: Vec2Arr | null, // Does it have a neighbor to the right side?
    bottom: Vec2Arr | null, // Does it have a neighbor to the bottom side?
    top: Vec2Arr | number | null, // Does it have a neighbor to the top side?
    BFSparent: Vec2Arr | number | null // Which tile is this tile's BFS parent
}
let world: WorldEntry[][] = []; // 2D world matrix
let dfsPath: Vec2Arr[] = []; // Depth-first search path
let bfsPath: Vec2Arr[] = []; // Breadth-first search path
...
function fadeIn(runtime: IRuntime) {
...
function worldBuildDFS(x: number, y: number, lastX: number, lastY: number) {
...
function explorerBFS(x: number, y: number, goalX: number, goalY: number) {
    // Walk through the world via Breadth-First Search pathfinding
    
    const q: Vec2Arr[] = []; // Queue
    ...
    while (q.length) {
        ...
        for (const n of neighbors) {
            if (n && n != NOWALL) {
                const nArr = n as Vec2Arr;
                if (!world[nArr[0]][nArr[1]].visited) {
                    world[nArr[0]][nArr[1]].visited = true;
                    world[nArr[0]][nArr[1]].BFSparent = [v[0], v[1]];
                    q.push(nArr);
                }
            }
        }
...
function traceBFSPath(e: Vec2Arr) {
    // Recursively visit the BFS-explored tiles parents to build a path array
    
    bfsPath.push(e);
    if (e && world[e[0]][e[1]].BFSparent)
        traceBFSPath(world[e[0]][e[1]].BFSparent as Vec2Arr);
}

function nextMove(runtime: IRuntime) {
