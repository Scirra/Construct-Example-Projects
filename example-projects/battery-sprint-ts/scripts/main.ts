...
let playerMod: InstanceType.PlayerModel;
let playerSpawn: InstanceType.PlayerSpawn;
let playerParticles: InstanceType.PlayerParticles;
let abyss: InstanceType.Abyss;
let levelFinish: InstanceType.LevelFinish;
let tbg: InstanceType.TiledBackground;

// These variables store list of instances
let cameraChanger: IObjectType<InstanceType.CameraChanger>;
let jumpPad: IObjectType<InstanceType.JumpPad>;

// Global objects
let camera: I3DCameraObjectType;

// Player behaviors
let pTween: ITweenBehaviorInstance<InstanceType.PlayerModel>; // Tween
let pPlat: IPlatformBehaviorInstance<InstanceType.PlayerCollider>; // Platformer

// Gameplay variables
let camZ: number; // Camera Z position
let camVecX: number; // Next camera up vector X
let currCamVecX: number; // Current camera up vector X
let tbgZElevation: number; // TiledBackground Z elevation
...
function restartGame(runtime: IRuntime) {
    ...
    runtime.getLayout(0).getLayer(0)!.backgroundColor = 
        tbg.effects[0].getParameter(0) as Vec3Arr;
...
function playerCollision(runtime: IRuntime) {
    ...
    for (const cc of cameraChanger.getAllInstances()) {
        if (cc.testOverlap(playerCol)) {
            ...
            const bgCol = tbg.effects[0].getParameter(+cc.instVars.bgFlipped) as Vec3Arr;
            setTimeout(() => {
...
function playerGlow(runtime: IRuntime) {
    ...
    if (pPlat.isOnFloor) {
        ...
        pFX.setParameter(
            2, lerp(pFX.getParameter(2) as number, 1, 10 * runtime.dt)
        );
    }
}
...
function flipBG(runtime: IRuntime) {
