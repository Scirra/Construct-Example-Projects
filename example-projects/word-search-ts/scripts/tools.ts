...
export function pointerPos(layer: IAnyProjectLayer, e: MouseEvent) {
    // Return pointer layer position.

    return layer.cssPxToLayer(e.clientX, e.clientY);
}

export function pointerOverObj(layer: IAnyProjectLayer, obj: IWorldInstance, e: MouseEvent) {
	// Check if pointer pointer is over a given object.

	return obj.containsPoint(...pointerPos(layer, e));
}

export function dist2D(x1: number, y1: number, x2: number, y2: number) {
...
export function lock2step(value: number, step: number) {
    // Lock value to step.

    return Math.round(value / step) * step;
}

export function pointFromAngle(x: number, y: number, angle: number, length: number) {
...
export function angle2direction(angle: number) {
