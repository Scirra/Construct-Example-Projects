...
let timer: ITimerBehaviorInstance<InstanceType.TimerManager>;
let statsText: InstanceType.ScoreText;
let gameOverText: InstanceType.GameOverText;
let gameOverMask: InstanceType.gameOverMask;
let playerMovementTween: ITweenState | null;

// List of all instances of a particular sprite
let playerPos: InstanceType.PlayerPos[];
let foodPos: InstanceType.FoodPos[];
let personPos: InstanceType.PersonPos[];
...
function onTimer(e: TimerBehaviorEvent, runtime: IRuntime) {
...
function resetPlayerAnimAndSpawnPlate(runtime: IRuntime) {
...
function breakFoodPlates(runtime: IRuntime) {
...
function makePersonGoAway(runtime: IRuntime) {
...
function checkPersonAndFoodCollision(runtime: IRuntime) {
...
function moveThePlayer(newAnimation: string) {
    // Helper function to declutter getPlayerInputs
    
    // Set Tween
    const newPosition: Vec2Arr = [
...
function onFoodAnimationEnd(e: SpriteAnimationEndEvent) {
...
function gameOver(runtime: IRuntime) {
    // Game over
